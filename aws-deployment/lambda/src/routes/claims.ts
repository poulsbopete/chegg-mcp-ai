...
router.get('/', async (req, res) => {
  try {
    ...
    if (from || to) {
      const rangeQuery: any = { range: { incidentDate: {} } };
    ...
    const searchBody = {
      ...
      from: parseInt(fromOffset as string),
    ...
    res.json({
      claims: result.hits.hits.map((hit: any) => ({
        id: hit._id,
        ...hit._source
      })),
      total: result.hits.total.value,
      page: Math.floor(parseInt(fromOffset as string) / parseInt(size as string)) + 1
    });
  ...
  try {
    ...
    if (claimData.claimType === 'auto' && claimData.vin) {
      ...
      if (existingClaims.hits.total.value > 0) {
        console.warn(`Duplicate VIN detected: ${claimData.vin}`);
...
router.get('/analytics', async (req, res) => {
  try {
    ...
    const analytics = result.aggregations.group_stats.buckets.map((bucket: any) => ({
      group: bucket.key,
